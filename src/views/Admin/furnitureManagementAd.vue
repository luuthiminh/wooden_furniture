<template>
  ...
  <div class="bg-white mb-2 rounded-xl mt-32">
    <div class="header pt-6 px-6 pb-10">
    ...
    <span class="text-sm font-medium text-gray-700 mx-6"
            ...
            <tbody>
              <tr v-for="f in furnitures" :key="f.furnitureId">
                ...
                >
                  ...
                  <template v-slot:body>
                    <div class="pb-3 px-4 text-sm text-left">
                      <!-- <div class="flex gap-x-6 mt-3">
                        <label
                          for="exampleInputEmail1"
                          class="form-label font-medium"
                ...
                >
                  ...
                  <template v-slot:body>
                    <p class="text-base py-3">
                      Are you sure detete <b> {{ furNameModal }} ?</b>
                    </p>
                  </template>
                  <template v-slot:footer>
  ...
  <p class="text-sm font-medium mb-5">Total users: {{ furnitures.length }}</p>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async opentModal(type, fur) {
      this.modalType = type;
      this.furNameModal = fur.furnitureName;
      this.furIdModal = fur.furnitureId;
      this.labelModal = fur.label;
      this.cateModal = fur.category;
      this.availableModal = fur.available;
      this.appropriateRoomModal = fur.appropriateRoom;
      this.priceModal = fur.price;
      this.imageModal = fur.image;
      this.collectionModal = fur.collection;
      this.collectIdModal = fur.collectionId;
      this.cateIdModal = fur.categoryId;
    },
    async getLabels() {
      try {
    ...
    async HandleUpdate() {
      const formData = new FormData();
      formData.append("FurnitureId", this.furIdModal);
      formData.append("FurnitureName", this.furNameModal);
      formData.append("CategoryId", this.cateIdModal);
      formData.append("LabelId", this.labelModal);
      formData.append("CollectionId", this.collectIdModal);
      formData.append("AppropriateRoom", this.appropriateRoomModal);
      try {
        const response = await axios.put(
          "shopOwner/shop-data/furnitures/edit",
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
        ...
        if (response.status === 200) {
          this.modalType = null;
          this.isAlertSuccess = true;
          this.messageSuccess = "Update " + this.furNameModal + " successful!";
          setTimeout(() => {
            this.isAlertSuccess = false;
          }, 5000);
          this.getFurnitures();
    ...
    async HandleDelete() {
      try {
        const response = await axios.delete(
          "shopOwner/shop-data/furnitures/delete/" + this.furIdModal
        );
        if (response.status === 204) {
          this.modalType = null;
          this.isSuccess = true;
          this.isAlertSuccess = true;
          this.messageSuccess = "Delete " + this.furNameModal + " successful!";
          setTimeout(() => {
            this.isAlertSuccess = false;
          }, 5000);
          this.getFurnitures();
